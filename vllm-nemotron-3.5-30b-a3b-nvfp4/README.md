...
| `MAX_NUM_QUEUED_REQS` | `32` | Admission cap: max in-flight requests (waiting + running); `32` = `4x MAX_NUM_SEQS` (`8`). Overflow -> HTTP `503` |
| `MAX_NUM_QUEUED_TOKENS` | `256K` | Admission cap: max queued prefill tokens counted conservatively (prefix-cache hits are not subtracted); `256K` = `32 x MAX_NUM_BATCHED_TOKENS`, one full batched-token window per admitted request. Overflow -> HTTP `503` |
