...
#   bash warmup.sh <model>      Warmup a specific model (qwen|muse|sglang|nemotron|gemma4)
#
...
case "$MODEL_SELECTOR" in
    qwen|muse|sglang|nemotron|gemma4) ;;
    *)
        echo "ERROR: Unknown model '${MODEL_SELECTOR}'. Valid models: qwen, muse, sglang, nemotron, gemma4."
        exit 1
        ;;
esac
...
for i in 1 2 3; do
    payload=$(printf '{"model":"%s","stream":false,"temperature":0.7,"max_tokens":64,"messages":[{"role":"user","content":"Warmup prompt #%d. What is 2+2?"}]}' "${SERVED_MODEL_NAME:-auto}" "$i")
...
payload=$(printf '{"model":"%s","stream":false,"temperature":0.3,"max_tokens":1024,"messages":[{"role":"user","content":"Solve: A water tank has two inlet pipes and one outlet pipe. Pipe A fills the tank in 3 hours, Pipe B in 5 hours, and the outlet empties it in 8 hours. If all three are open simultaneously, how long to fill an empty tank? Show step by step."}],"chat_template_kwargs":{"enable_thinking":%s}}' "${SERVED_MODEL_NAME:-auto}" "$think_flag")
post "$payload" 180 "Reasoning path (thinking=${think_flag})"

# ── Phase 7: Tool calling path warmup ───────────────────────────────────────
log "Phase 7: Tool calling path warmup..."
payload=$(printf '{"model":"%s","stream":false,"temperature":0.3,"max_tokens":256,"messages":[{"role":"user","content":"What is the weather in London?"}],"tools":[{"type":"function","function":{"name":"get_weather","description":"Get weather for a city","parameters":{"type":"object","properties":{"city":{"type":"string"}},"required":["city"]}}}]}' "${SERVED_MODEL_NAME:-auto}")
post "$payload" 60 "Tool calling path"

# ── Phase 8: Streaming path warmup ──────────────────────────────────────────
log "Phase 8: Streaming path warmup..."
payload=$(printf '{"model":"%s","stream":true,"temperature":0.7,"max_tokens":256,"messages":[{"role":"user","content":"Write a haiku about artificial intelligence."}]}' "${SERVED_MODEL_NAME:-auto}")
