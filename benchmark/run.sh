...
#   bash run.sh <model>               Run all benchmarks for a model (qwen|muse|sglang|nemotron|gemma4)
...
# $1 = model selector (qwen|muse|sglang|nemotron|gemma4, default qwen). Backward compatibility: if $1
    ...
    case "$1" in
        ...
        *)
            if [[ -z "${2:-}" && -f "${TESTS_DIR}/$1.sh" ]]; then
                ...
                echo "  model : qwen | muse | sglang | nemotron | gemma4 (default: qwen)"
